...
// fault could in theory be counted twice here, but that's vanishingly rare and
// far outweighed by un-splitting the categories.
function normalizeFaults(s: CallingStats): CallingStats {
    const byFaultMap = new Map<string, { k: string; n: number; attempts: number }>();
    for (const r of s.byFault) {
        const k = canonicalFault(r.k);
        const cur = byFaultMap.get(k) ?? { k, n: 0, attempts: 0 };
        cur.n += r.n;
        cur.attempts += r.attempts;
        byFaultMap.set(k, cur);
    }
    const rollUp = <T extends { c: string; n: number }>(rows: T[], groupKey: (r: T) => string) => {
        const m = new Map<string, T>();
        for (const r of rows) {
            const c = canonicalFault(r.c);
    ...
    return {
        ...s,
        byFault: [...byFaultMap.values()].sort((a, b) => b.n - a.n || a.k.localeCompare(b.k)),
        faultByFeeder: rollUp(s.faultByFeeder, (r) => (r as { f: string }).f),
        faultBySubstation: rollUp(s.faultBySubstation, (r) => (r as { s: string }).s)
    };
}
    ...
    return (Chart: any, el: HTMLCanvasElement) =>
        new Chart(el, {
            ...
            options: {
                ...
                // Horizontal bars, so rows are picked along y. Chart.js defaults
                // the interaction axis to 'x' whatever indexAxis says - without
                // this an index-mode tooltip resolves the pointer by bar length
                // and reports a different row's numbers.
                ...
                plugins: {
                    ...
                    tooltip: { ...TOOLTIP, mode: 'index', axis: 'y', intersect: false }
                },
                scales: {
...
function CallingReport() {
    ...
    const exportExcel = async () => {
        ...
        try {
            ...
            headerStyle(types.addRow(['Complaint Type', 'Total', 'Called', 'Connected', 'Connect % (of called)']));
            stats.byType.forEach((t) => types.addRow([t.k, t.n, t.called, t.connected, `${pct(t.connected, t.called)}%`]));
            types.addRow([]);
                                ...
                                <table className="w-full text-left text-sm">
                                    <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 font-semibold text-right">Connect % <span className="normal-case text-gray-400">(of called)</span></th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100">
                                        {stats.byType.map((t) => (
                                            <tr key={t.k} className="hover:bg-sky-50/50 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-right text-gray-700">{pct(t.connected, t.called)}%</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                ...
                {subTab === 'substations' && (
                    <>
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 text-sm text-gray-600">
                            ...
                            it comes from the live feed and is synced for every complaint.
                        </div>
                        {ssItems.length === 0 ? (
                            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-8 text-center">
                                <p className="text-lg font-bold text-amber-900">No substation data in this range</p>
                                <p className="mt-2 text-sm text-amber-800">No complaints with a known substation arrived in this date range.</p>
                            </div>
                        ) : (
                            <>
                                <ChartCard
