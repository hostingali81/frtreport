...
class _ComplaintsScreenState extends State<ComplaintsScreen> with WidgetsBindingObserver {
  ...
  Future<void> _fetchLatest() async {
    ...
    try {
      // Always pull from FRT. This briefly skipped the pull when the grid looked
      // fresh enough, to save the Vercel invocation — which was the wrong trade:
      // an operator presses Refresh precisely because they want the newest
      // complaints RIGHT NOW, and a complaint filed a minute after the last
      // background cycle would not appear. An explicit action has to do what it
      // says. The saving belongs in the automatic paths (Realtime, and the
      // refresh loop keeping the grid fresh so this gets pressed less often),
      // never in second-guessing a button the operator just tapped.
      await Api.sync();
      await _load();
      Haptics.success();
    } catch (e) {
