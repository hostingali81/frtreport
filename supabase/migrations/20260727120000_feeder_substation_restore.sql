-- Restore substation-aware feeder grouping in get_complaints_stats.
--
-- Migration 20260706170000 changed the feeder grouping set to (sub_station,
-- feeder) and added 'ss' to byFeeder, because the same feeder name can exist
-- under two substations and those are different feeders. Migration
-- 20260708120000 (drop raw_data) rewrote the whole function from the older
-- pre-substation copy, silently reverting both changes - so the Feeder Report
-- has been showing "Unknown" in its Substation column ever since.
--
-- This is 20260708120000's body with the feeder grouping fixed again:
--   * grouping set (feeder) -> (sub_station, feeder)
--   * byFeeder carries 'ss'
--   * bySubStation needs "AND feeder IS NULL" now that the feeder rows also
--     carry a non-null sub_station (without the guard every feeder row would
--     be counted as a substation row).
