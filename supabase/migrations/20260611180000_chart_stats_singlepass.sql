-- Make get_complaints_stats fit comfortably inside the statement timeout on
-- the full dataset. Profiling (EXPLAIN ANALYZE) showed two hotspots:
--   1. per-row to_char()/AT TIME ZONE expressions (~22us/row adds seconds at
--      80k rows) - to_char is now used only on the handful of grouped month
--      rows, res_mins subtracts timestamptz directly (no tz conversion), and
--      the month key comes from substr(date::text) instead of to_char;
--   2. ~14 separate scans of the materialized CTE - all dimensional
--      groupings are now computed in ONE pass via GROUP BY GROUPING SETS,
--      plus one plain pass each for the two repeat-consumer groupings.
-- Grouping-set rows are identified by NULL patterns: every dimension column
-- is non-null in enriched (coalesced to 'Unknown'), so a NULL means "not
-- part of this grouping set".
...
enriched AS MATERIALIZED (
    SELECT
        id, division, sub_station, area_type, status_l, closed_status_t,
...
g AS MATERIALIZED (
    SELECT
        division, sub_station, area_type, d, mkey, dow, hr,
        count(*) AS n,
...
SELECT jsonb_build_object(
    'total', (
        ...
        WHERE division IS NULL AND sub_station IS NULL AND area_type IS NULL
          AND d IS NULL AND mkey IS NULL
    ),
    'byDivision', (
