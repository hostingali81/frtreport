...
-- (possibly to '') everywhere else and NULL in all other sets.
...
g AS MATERIALIZED (
    SELECT
        division, sub_station, area_type, closed_status_t AS closed_status_g, d, mkey, dow, hr,
        count(*) AS n,
...
SELECT jsonb_build_object(
    'total', (
        ...
        WHERE division IS NULL AND sub_station IS NULL AND area_type IS NULL
          AND closed_status_g IS NULL AND d IS NULL AND mkey IS NULL
    ),
    'byClosedStatus', (
