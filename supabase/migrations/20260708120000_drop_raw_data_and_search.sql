-- Drop old functions that have p_search parameter
DROP FUNCTION IF EXISTS public.get_complaints_page(text, text, text, text, text, timestamptz, timestamptz, text, timestamptz, bigint, integer);
DROP FUNCTION IF EXISTS public.get_complaints_stats(text, text, text, text, text, timestamptz, timestamptz, text);

-- Drop indexes and columns
DROP INDEX IF EXISTS public.idx_complaints_search_trgm;
ALTER TABLE public.complaints DROP COLUMN IF EXISTS search_text;
ALTER TABLE public.complaints DROP COLUMN IF EXISTS raw_data;

-- Recreate get_complaints_page without p_search and mapping individual columns
...
BEGIN
    ...
    EXECUTE format(
        'WITH page AS (
            SELECT 
                jsonb_build_object(
                    ...
                    ''Closed By'', closed_by,
                    ''Complaint Date and Time'', to_char(complaint_date at time zone ''Asia/Kolkata'', ''MM/DD/YYYY HH12:MI AM''),
                    ''Closed Date'', CASE WHEN closed_date IS NOT NULL THEN to_char(closed_date at time zone ''Asia/Kolkata'', ''MM/DD/YYYY HH12:MI AM'') ELSE NULL END,
        ...
        SELECT jsonb_build_object(
            ''rows'', coalesce((SELECT jsonb_agg(mapped_data ORDER BY complaint_date DESC, id DESC) FROM page), ''[]''::jsonb),
...
-- Recreate get_complaints_stats without p_search and using explicit columns instead of raw_data
...
enriched AS MATERIALIZED (
    SELECT
        id, complaint_number, division, sub_station, area_type, feeder, status_l, status, closed_status_t,
        closing_remarks, mobile, cname, caddr, ts, complaint_date, closed_date,
        ts::date AS d,
...
mob_lists AS (
    SELECT e2.mobile,
           jsonb_agg(jsonb_build_object(
               'Complaint Number', e2.complaint_number,
               'Division', e2.division,
               'Sub Station', e2.sub_station,
               'Feeder', e2.feeder,
               'Complaint Date and Time', to_char(e2.complaint_date at time zone 'Asia/Kolkata', 'MM/DD/YYYY HH12:MI AM'),
               'Status', e2.status,
               'Closed Status', e2.closed_status_t,
               'Closed Date', CASE WHEN e2.closed_date IS NOT NULL THEN to_char(e2.closed_date at time zone 'Asia/Kolkata', 'MM/DD/YYYY HH12:MI AM') ELSE NULL END,
               'Closing Remarks', e2.closing_remarks
...
SELECT jsonb_build_object(
    ...
    'bySubStation', (
        ...
        FROM g WHERE sub_station IS NOT NULL AND mkey IS NULL
    ),
    'byAreaType', (
    ...
    'byFeeder', (
        SELECT coalesce(jsonb_agg(jsonb_build_object(
                   'k', feeder, 'n', n, 'pending', pending, 'beyond', beyond,
                   'resSum', res_sum, 'resN', res_n
               ) ORDER BY n DESC, feeder), '[]'::jsonb)
