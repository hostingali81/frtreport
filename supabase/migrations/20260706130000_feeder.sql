-- The FRT report recently started including a Feeder column. Promote it out of
-- raw_data into a real column (backfilled for the rows that already have it),
-- and add a byFeeder aggregation to get_complaints_stats for the new
-- feeder-wise report. The scraper now writes `feeder` on upsert; note that
-- adding the key to the upsert record changes every content_hash once, so the
-- first scrape after deploy rewrites the rows inside its window (expected).

ALTER TABLE public.complaints ADD COLUMN IF NOT EXISTS feeder text;

UPDATE public.complaints
SET feeder = nullif(trim(raw_data->>'Feeder'), '')
WHERE nullif(trim(raw_data->>'Feeder'), '') IS NOT NULL
  AND feeder IS DISTINCT FROM nullif(trim(raw_data->>'Feeder'), '');

-- get_complaints_stats: one more grouping set (feeder) computed in the same
-- single pass, plus a `pending` aggregate so the feeder report can show
-- pending/closed splits. Feeder is coalesced to 'Unknown' like the other
-- dimensions; byFeeder excludes the Unknown bucket (old rows scraped before
-- the column existed dominate it and it is noise in a feeder report) - the
-- client derives coverage as sum(byFeeder n) vs total.
